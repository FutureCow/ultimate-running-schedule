  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <ComposedChart data={data} margin={{ top: 4, right: secondary ? 42 : 8, left: 0, bottom: 0 }}>
        <defs>
          <linearGradient id={`grad-friend-${primary.key}`} x1="0" y1="0" x2="0" y2="1">
        ...
        {primary.type === "area" ? (
          <Area
            ...
            fill={`url(#grad-friend-${primary.key})`}
            strokeWidth={2}
            dot={false}
            connectNulls
          />
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
          ...
          <button onClick={onClose} className="text-slate-400 hover:text-white transition-colors p-2 rounded-xl hover:bg-slate-800">
            <X className="w-5 h-5" />
          </button>
        </div>
...
export default function FriendActivityDetailPage() {
  const params = useParams<{ friendId: string; activityId: string }>();
  ...
  const { data, isLoading, error } = useQuery<ActivityDetail>({
    queryKey: ["friend-activity", friendId, activityId],
    queryFn: () => friendsApi.getActivity(friendId, activityId).then((r) => r.data),
      ...
      <div className="min-h-screen lg:pl-60">
        ...
        <main className="px-4 py-6 pb-24 lg:pb-6 max-w-5xl mx-auto space-y-5">
          <div className="flex items-center gap-3">
            <Link href={`/friends/${friendId}`} className="btn-ghost px-2 py-2">
          ...
          {data && (
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                <StatCard icon={<Ruler className="w-3.5 h-3.5" />} label={t("stats.distance")} value={`${data.summary.distance_km} km`} />
                <StatCard icon={<Clock className="w-3.5 h-3.5" />} label={t("stats.duration")} value={fmtTime(data.summary.duration_seconds)} />
                <StatCard icon={<Zap className="w-3.5 h-3.5" />} label={t("stats.pace")} value={data.summary.avg_pace_per_km ? `${data.summary.avg_pace_per_km} /km` : "–"} />
                <StatCard
                ...
                />
                <StatCard icon={<Activity className="w-3.5 h-3.5" />} label={t("stats.cadence")} value={data.summary.avg_cadence ? `${data.summary.avg_cadence} spm` : "–"} />
                <StatCard icon={<TrendingUp className="w-3.5 h-3.5" />} label={t("stats.elevation")} value={data.summary.elevation_gain_m ? `${Math.round(data.summary.elevation_gain_m)} m` : "–"} />
              </div>
