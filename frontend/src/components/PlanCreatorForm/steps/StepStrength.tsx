...
export function StepStrength({ watch, setValue }: Props) {
  ...
  const strengthDays = watch("strength.days") ?? [];

  function toggleStrengthDay(dayIndex: number) {
    const current = strengthDays as number[];
    if (current.includes(dayIndex)) {
      setValue("strength.days", current.filter((d) => d !== dayIndex));
    } else {
      setValue("strength.days", [...current, dayIndex]);
    }
  }
        ...
        <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              {LOCATION_KEYS.map((key) => (
                <button
                  key={key}
                  type="button"
                  onClick={() => setValue("strength.location", key)}
                  className={cn(
          ...
          <div>
            ...
            <div className="grid grid-cols-7 gap-1.5">
              {DAYS_EN.map((_, i) => {
                const dayNum = i + 1;
                const selected = (strengthDays as number[]).includes(dayNum);
                return (
                  <button
                    key={dayNum}
                    type="button"
